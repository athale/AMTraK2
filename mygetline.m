...
% Latest modification: Anushree R. Chaphalkar 17.10.2016
...
%   $Revision: 5.27.4.5 $  $Date: 2008/12/22 23:42:54 $
...
state= uisuspend(GETLINE_FIG); %, 0);%18/3/2018
...
if (errCatch == 1)
    errStatus = 'trap';
    eid = 'Images:getline:interruptedMouseSelection'; %ARC
    error(eid, '%s', 'Interruption during mouse selection.'); %ARC
    return %ARC added 31/07/2019
    
elseif (~ishghandle(GETLINE_H1) || ...
            ~strcmp(get(GETLINE_H1, 'UserData'), 'Completed'))
    errStatus = 'unknown';
    errordlg('LOI selection interrupted', 'Selection Error'); %ARC 17/10/2016
   return %ARC added 31/07/2019
else
    errStatus = 'ok';
    x = GETLINE_X(:);
