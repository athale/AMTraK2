...
dx=x(2:end)-x(1:end-1); %first derivative
r=find(dx>0); %positions
f=find(dx<0); %positions

if ~isempty(r) & ~isempty(f)  %ARC  % we must have at least one rise and one fall
    dr=r;
    dr(2:end)=r(2:end)-r(1:end-1) ; %second derivative-arc says
    rc=ones(nx,1) ; %ARC
    rc(r+1)=1-dr;%%%??
    rc(1)=0;
    ...
    %If time since the last rise is smaller than 
    %time since the next fall, 
    %if time to the next rise is greater than 
    %time to the next fall  %ARC 
    v=x(k) ;
